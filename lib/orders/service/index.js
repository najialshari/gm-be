    ...
    try {
        ...
        if (tableUUID) {
            ...
            if (tableObj) {
                ...
                if (created) {
                    ...
                    if (newDetail) return response.success('Local order created successfully.', newDetail, res)
                }
        ...
        if (userId && addressId) {
            ...
            if (data) {
                ...
                const newDetail = await models.OrderDetail.bulkCreate(orderDetails)
                if (newDetail) return response.success('New online order created successfully.', newDetail, res)
            }
...
const closeOrder = async (req, res, next) => {
    try {
        ...
        if (updated[0] === 1) return response.success('Order closed successfully.', req.params.id, res)
        else return response.failedWithMessage('Order not updated!!!', res)
    } catch (err) {
        console.log('[orders/service]ERROR--->', err)
        return response.failedWithMessage('Server error!!!', res)
