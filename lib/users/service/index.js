...
const updateUsername = async (currUser, newUsername) => {
  try {
    const updatedUsername = await models.User.update(
      {
    ...
    if (updatedUsername[0] == 1) return updatedUsername;
    else return false;
  } catch (e) {
    console.error(e);
    throw new Error(e);
...
const updateEmail = async (currUser, newEmail) => {
  try {
    const updatedEmail = await models.User.update(
      {
        email: newEmail,
      },
      {
    ...
    if (updatedEmail[0] == 1) return updatedEmail;
    else return false;
  } catch (e) {
    console.error(e);
    throw new Error(e);
...
const updateUserInfo = async (currUser, newUsername, newEmail, newPhoneNo) => {
  try {
    const newUser = await models.User.update(
      {
        username: newUsername,
