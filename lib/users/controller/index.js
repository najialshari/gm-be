...
const signup = async (req, res) => {
  try {
    const { username, phoneNo, password, passwordConfirmation } =
      req?.body;
...
const updatePassword = async (req, res) => {
  try {
    ...
    if (!currPassword || !newPassword || !newPasswordConfirmation) {
      return responses.failedWithMessage(
        "please do not make the fields empty",
        res
      );
    }
...
const updateProfilePic = async (req, res) => {
  try {
    ...
    console.log(" inserted file name :"+uniqueFileName)    
    console.log("here after insert the file")    
    await firebase.uploadBytes(imageRef, file?.buffer, metaType)
      console.log("here after Uplaud the file to firebase")  
      .then(async () => {
