...
const updateUsername = async (req, res) => {
  try {
    ...
    if (newUsername?.length < 3)
      return responses.failedWithMessage(
        "Username minimum lenght is invalid",
        res
      );
    ...
    if (isNewUsernameAlreadyExists)
      return responses.failedWithMessage(
        "This Username was taken! Please use another one",
        res
      );
    ...
    return responses.failedWithMessage(
      "Unknown fail!. Contact app developer",
      res
    );
  } catch (err) {
    console.log(err);
...
const updateEmail = async (req, res) => {
  try {
    ...
    if (isNewEmailAlreadyExists)
      return responses.failedWithMessage(
        "This Email was taken! Please use another one",
        res
      );
    if (!authService.comparePasswords(password, currUser.password))
      return responses.failedWithMessage(
...
const resetPass = async (req, res) => {
  ...
  if (!email) return responses.failedWithMessage("Please add required Email first!",res);
  const isEmail = isEmailValid(email);
  if (!isEmail)
    return responses.failedWithMessage("Email not Valid. Use valid Email!",res);
  if (email && isEmail) {
    const isExistUser = await models.User.findOne({ email });
    if (!isExistUser)
      return responses.failedWithMessage("Email not found. Use your original Email!",res);
    const newPassword = await service.resetPassword(isExistUser);
    if (!newPassword)
      return responses.failedWithMessage("Could not reset password. Please try again.",res);
    else return responses.successWithMessage("Congratulation. New password is 123456", res);
  }
  return responses.failedWithMessage("Something went wrong. Please try again or contact the Admin.",res);
};
