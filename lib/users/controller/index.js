...
const updateUserInfo = async (req, res) => {
  try {
    ...
    if (result) {
      const updatedUser = await models.User.findByPk(currUser?.id);
...
const updatePassword = async (req, res) => {
  try {
    ...
    if (!authService.comparePasswords(currPassword, currUser?.password))
      return responses.failedWithMessage("Please enter your current password!", res);
    ...
    if (authService.comparePasswords(newPassword, currUser?.password))
      return responses.failedWithMessage(
        "Your new and old password can not be the same",
        res
      );
