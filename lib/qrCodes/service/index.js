...
const getTables = async (req, res, next) => {
  try {
    ...
    else return responses.successWithMessage("No tables found!!!", res);
  } catch (err) {
    ...
    if (tableData) {
      ...
      if (token) {
        ...
        return responses.success(
          "QR Code Scaned in successfully",
          { table: tableData, token },
          res
        );
      }
      return responses.failedWithMessage(
        "Something went wronge with Token ",
        res
      );
    }
...
const getBusyTables = async (req, res, next) => {
  try {
    ...
    else return responses.successWithMessage("No orders found!!!", res);
  } catch (err) {
    console.log("[orders/service]ERROR--->", err);
    return responses.failedWithMessage("Server error!!!", res);
  }
...
const deleteTable = async (req, res, next) => {
  try {
    ...
    if (deleted[0] === 1)
      return responses.success("Table deleted successfully.", deleted, res);
    else return responses.successWithMessage("Table not existed!!!", res);
  } catch (err) {
    console.log("[orders/service]ERROR--->", err);
    return responses.failedWithMessage("Server error!!!", res);
  }
...
const updateTable = async (req, res, next) => {
  try {
    ...
    if (!tableExisted)
      ...
      if (updated[0] === 1)
        return responses.success("Table updated successfully.", updated, res);
